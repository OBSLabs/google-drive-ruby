...
module GoogleDrive
    ...
    class Worksheet
        ...
        def save(chunk_size = 25, retries = 5)
          ...
          if !@modified.empty?
            ...
            @modified.each_slice(chunk_size) do |chunk|
              puts "Chunk #{chunk_num} / #{1+@modified.length/chunk_size}"
              chunk_num += 1
              ...
              result = @session.request(:post, batch_url, :data => xml, :header => {"Content-Type" => "application/atom+xml;charset=utf-8", "If-Match" => "*"}, :retries => retries)
